...
import com.cgj.accountbook.dao.DatabaseOutput;
...
public class ActivitySetting extends AppCompatActivity implements OnClickListener {
    ...
    private void initData() {
        ...
        int gdkx = databaseUtil.findAll(LimitsDatabase.class).size();
        int lbgl = databaseUtil.findAll(LimitsDatabase.class).size();
        int qksysj = databaseUtil.findAll(AccountDatabase.class).size() - 1;
        ...
        set_tv_gdkx.setText(String.format(getString(R.string.setting_string_kaixiao), gdkx));
        set_tv_myxe.setText(MyStringUtils.readSharedpre(this, 1) + " "
                + getString(R.string.rmb));
        set_tv_lbgl.setText(String.format(getString(R.string.setting_string_leibie), lbgl));
        set_tv_glsr.setText("");
        set_tv_dcjl.setText("SD卡/Account");
        set_tv_qksysj.setText(String.format(getString(R.string.setting_string_qingkong), qksysj));
    }

    private void showSettingDialog(final int type) {
        // TODO showDialog
        LayoutInflater layoutInflater = getLayoutInflater();
        View dialog = layoutInflater.inflate(R.layout.activity_setting_dialog, (ViewGroup) findViewById(R.id.setting_dialog));
        final EditText eText = (EditText) dialog.findViewById(R.id.setting_dialog_et);
        final TextView tView = (TextView) dialog.findViewById(R.id.setting_dialog_tv);
                    ...
                    switch (type) {
                        case 1:
                            ...
                            if (value.length() == 9) {
                                ...
                                MyStringUtils.saveSharedpre(ActivitySetting.this, 2, value);
                                set_tv_psw.setText("已开启");
                                Toast.makeText(ActivitySetting.this, "设置成功！", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(ActivitySetting.this, "密码格式不正确！", Toast.LENGTH_SHORT).show();
                            }

                            break;
                        case 2:
                            value = MyStringUtils.get2dotFloat(Float.parseFloat(value));
                            MyStringUtils.saveSharedpre(ActivitySetting.this, 1, value);
                            initData();
                            break;
                    }
                }
                ...
                builder.setNeutralButton("取消启动密码",
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                Toast.makeText(ActivitySetting.this, "取消密码成功！", Toast.LENGTH_SHORT).show();
                            }
                        });
                builder.show();
                break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.set_rl2:
                // 与财政预算那里的修改每个限额相关联
                startActivityForResult((new Intent(ActivitySetting.this, ActivityMaGdkx.class)), 1);
                break;
            case R.id.set_rl3:
                showSettingDialog(2);
                break;
            case R.id.set_rl4:
                // 类别管理
                startActivityForResult((new Intent(ActivitySetting.this, ActivityMaLeibie.class)), 1);
                break;
            case R.id.set_rl5:
                // 弹出收入活动，可以添加修改删除的每个listview的活动
                startActivityForResult((new Intent(ActivitySetting.this, ActivityMaIncome.class)), 1);
                break;
            case R.id.set_rl6:
                // 导出消费记录
                String path = "";
                ...
                dataBase.close();
                Toast.makeText(ActivitySetting.this, "记录导出成功！导出至：/n" + path, Toast.LENGTH_LONG).show();
                break;
            case R.id.set_rl7:
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
